			...
			if(_varName == 'objectsHQ') exitWith {
				{
					...
					switch (_type) do {
						...
						case "caja": {
							...
							caja setPosATL ((_x select 1) vectorAdd [0, 0, 60]); //Sparker
						};
						case "cajaveh": {
							cajaveh setDir (_x select 2);
							cajaveh setPosATL ((_x select 1) vectorAdd [0, 0, 60]);
						};
						case "fuego": {
						...
						case "mapa": {
							...
							mapa setPosATL ((_x select 1) vectorAdd [0, 0, 60]);
						};
						case "petros": {
