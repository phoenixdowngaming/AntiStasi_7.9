...

{
	...
	if (_objective in ciudades) then {
		...
		if (!(_base == "") or !(_airport == "")) then {
			...
			//if (debug) then {hint format ["Marcador: %1. ScoreneededLand: %2. ScoreneededAir: %3. ScoreLand: %4. ScoreAir: %5",_objective, _scoreNeededLand, _scoreNeededAir,_scoreLand,_scoreAir]; sleep 5};
