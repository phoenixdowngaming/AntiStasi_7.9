...
createAAsite = compile preProcessFileLineNumbers "CREATE\createAAsite.sqf";  //Stef 18/08 can we add an if _prestigeCSAT>50 true, false (sort of) here?
...
INT_Reinforcements = compile preProcessFileLineNumbers "Missions\INT_Reinforcements.sqf";  //Stef 29/07 Too difficult to fix, better version in WotP already Sparker: need to look closer at this
...
//AS_forest = compile preProcessFileLineNumbers "Missions\AS_ForestPatrol.sqf";  disabled because they're oftenly messy, it add 10 units to the pool mostly for no reason.
