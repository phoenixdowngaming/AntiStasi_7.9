...
{
	diag_log format ["fn_putRoadblockMarkerAtFrontline.sqf: controlesCounter not initialized! Removing previous roadblock markers.", _rbpos, _rbname, _rbpos];
...
for [{private _i = 0}, {_i < ws_gridSizeX}, {_i = _i + 1}] do //_i is x-pos
{
	for [{private _j = 0}, {_j < ws_gridSizeY}, {_j = _j + 1}] do //_j is y-pos
	{
		...
		if(_value != 0) then //Try to put a roadblock here
		{
			...
			_rb = [_gridPos, 1.4*_halfSize, 10, _frontlineDir, 60, _newRbMarkers + _keepoutMarkers, _keepoutRadius, _debug] call ws_fnc_findRoadblockRoads;
